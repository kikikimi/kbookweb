...
public class CarModelOptionsIO extends Thread implements SocketClientInterface, SocketClientConstants{
	...
	public void sendCmd (String cmd) {
		...
		if (objInput.equals("OK") || objInput.equals("WELCOME"))  {
			try {
				writer.writeObject(cmd);//UPLOAD, CONFIGURE, or QUIT
			}
			catch (IOException e) {System.out.println("Problem sending command " + cmd);}
		}
	}
	@SuppressWarnings("unchecked")
	public ArrayList<String> getModels () {
		...
		Object modelNames;
		ArrayList<String> modelStrings = null;
		sendCmd("CONFIGURE");
		try { 
			modelNames = reader.readObject();
			System.out.println(modelNames);
			if (modelNames instanceof ArrayList <?>){
				modelStrings = (ArrayList<String>)modelNames;
			}
		}
		catch (IOException ie) {}
		catch (ClassNotFoundException ce) {}
	...
	public Model getModelOptions(String modName) {
		try { 
			writer.writeObject(modName); 
			return buildAuto (this.reader);
		}
